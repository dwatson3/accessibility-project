...
function WebSpeechRecognition(w) {
  var r = this;
  r.window = w;
  r.continuous = false;
  r.lang = '';
  ...
  r.onEnd = function() {
  if (r.create_email_on_end) {
    r.create_email_on_end = false;
    r.createEmail(r.final_transcript);
  }
};

  // Handler when user clicks "Create Email" button.
  r.emailButton = function() {
  // if (r.inProgress()) {
  //   console.log('waiting');
  //   // Wait for recognition to end before calling createEmail().
  //   r.create_email_on_end = true;
  //   r.stop();
  // } else {
...
// Create email by splitting string s into subject and body.
  ...
  r.window.location.href = 'mailto:?subject=' + subject + '&body=' + body;
};
    ...
    r.recognition.onresult = function(event) {
      ...
      for (var i = event.resultIndex; i < event.results.length; ++i) {
        if (event.results[i].isFinal) {
          // look for 'write email'
          var tempResults = event.results[i][0].transcript;
          if (tempResults.trim().toLowerCase() === 'write email') {
